...
def load_corpus():
    ...
    return load_or_prepare_corpus(get_settings())


@st.cache_resource(show_spinner="Нарезаю на чанки...")
def load_chunks(strategy: str, size: int, overlap: float, table_mode: str):
    ...
    return chunk_pages(load_corpus(), config, settings.embedding.model_name)


def raw_text(page) -> str:
    settings = get_settings()
...
def main() -> None:
    ...
    chunks = load_chunks(strategy, size, overlap, table_mode)
    ...
    with left:
        ...
        st.code(raw_text(page)[:6000], language="text")
    with right:
