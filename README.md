...
![Дашборд: сетка чанкинга 256/384/512 × оверлап](docs/screenshots/dashboard.png)

![Дашборд: эмбеддинги — качество и задержка поиска](docs/screenshots/dashboard-embeddings.png)

![Дашборд: этап E и бейслайн vs улучшенный на 76 ручных](docs/screenshots/dashboard-retrieval.png)

![Дашборд: локальные LLM на 76+12](docs/screenshots/dashboard-llm.png)
...
**Гипотезы.** (1) Вторая коллекция строк таблиц даст больше, чем ещё одна эмбеддинг-модель. (2) Весь корпус в `rows` хуже dual-index. (3) Без крошек лучше. (4) Лексическое раскрытие без LLM поможет: к запросу дописываются слова справки по 5 шаблонам (`клавиш` → «горячая клавиша сочетание клавиш», меню/права/ветки/OLE), максимум две добавки, затем RRF.
